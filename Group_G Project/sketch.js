...
let bg; // A layer used for storing the background of a static oil painting
let textureOverlay; // A layer used for storing the texture of the canvas

// An array of vertices of the body parts of a bull
...
function setup() {
  ...
  // 1. Create a static oil painting background
  bg = createGraphics(width, height); 
  createImpastoBG(); 

  // 2. Create one-time, programmatic canvas textures
  textureOverlay = createGraphics(width, height);
  createGrainTexture(textureOverlay);

  // Initialize the vertex data of the bull at one time in setup
  body = [
...
function draw() {
  // 1. Draw the pre-generated background layer
  image(bg, 0, 0, width, height);
  
  // 2. Draw a moving bull
  drawCow();
  
  // 3. After all the contents have been drawn, overlay the texture layers in the blending mode
  push();
  blendMode(OVERLAY); // Try OVERLAY, SOFT_LIGHT, or MULTIPLY
  image(textureOverlay, 0, 0);
  blendMode(BLEND); // Reset the blending mode to avoid affecting the next frame
  pop();
}
...
// added: Function for creating programmatic noise textures
function createGrainTexture(graphics) {
  const grainAmount = 100000; // The number of noise points
  graphics.noStroke();
  for (let i = 0; i < grainAmount; i++) {
    const x = random(width);
    const y = random(height);
    // Draw a very small, semi-transparent white or black noise point
    const alpha = random(0, 15);
    if (random() > 0.5) {
      graphics.fill(255, alpha); // White noise point
    } else {
      graphics.fill(0, alpha);   // Black noise point
    }
    graphics.rect(x, y, 1, 1);
  }
}

// The drawing logic of the bull
function drawCow() {
...
// Programmed oil painting background generation function
...
// The browser window is adjusted reactively
